...
export const FormModalProfile = ({clickOn,dataPar}) => {
  const [datas, setData] = useState({ username:null ,email: null, password: null });
  ...
  const postFormSign = (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.REACT_APP_API_URL}/api/v1/auth/profile`, {
        email: datas.email,
    ...
    <>
      ...
      <div className=" z-[999]  fixed bottom-2 right-0 left-0   w-screen h-max ">
        ...
        >
          ...
          <div className="flex text-[17px] w-full gap-2 h-max flex-col font-['poppins'] hidden">
            <label className=" font-medium" htmlFor="">
              Email
            </label>
            <input
          ...
          <div className="text-[17px] flex w-full gap-2 h-max flex-col font-['poppins'] hidden">
            <label className=" font-medium" htmlFor="">
              Password
            </label>
            <input
